...
## 更新摘要
**变更内容**
- 新增 pwd_image 表结构设计，支持图像附件功能
- 更新数据库架构概览，包含图像表的关联关系
- 添加图像表的业务逻辑说明和数据完整性约束
- 更新依赖分析，包含图像处理相关的组件
...
本文件详细说明密码管理器中各个数据表的结构设计，包括config、group、pwd_info、oss_config、shortcut_key、pwd_image等表的字段定义、数据类型和约束规则。文档记录了主键、外键关系和索引设计，解释了表之间的关联关系和数据完整性约束，提供了完整的数据库schema图表和示例数据，说明了表设计的业务逻辑依据和扩展性考虑。
...
### 表结构初始化
- **initSql.ts**: 定义所有数据表的创建语句和初始数据插入逻辑，包含新增的图像表
...
- [db.ts:1-30](file://electron/db/sqlite/components/db.ts#L1-L30)
- [baseSql.ts:1-87](file://electron/db/sqlite/components/baseSql.ts#L1-L87)
- [initSql.ts:107-174](file://electron/db/sqlite/components/initSql.ts#L107-L174)
...
**新增** pwd_image表用于存储密码信息的图像附件，支持加密的Base64图像数据存储、MIME类型处理、文件大小跟踪、排序管理等功能。
...
PWD_INFO ||--o{ PWD_IMAGE : "contains"
```

**图表来源**
- [initSql.ts:107-120](file://electron/db/sqlite/components/initSql.ts#L107-L120)
- [initSql.ts:159-173](file://electron/db/sqlite/components/initSql.ts#L159-L173)
- [image.ts:38-67](file://electron/db/sqlite/mapper/image.ts#L38-L67)

**表结构详情**：
- **主键**: id (INTEGER, 自增)
- **字段定义**:
  - id: 整数类型，自动递增主键
  - pwd_id: 整数类型，关联到pwd_info表的密码信息ID
  - file_name: 文本类型，文件原始名称
  - file_size: 整数类型，文件大小（字节）
  - mime_type: 文本类型，MIME类型标识符
  - data: 文本类型，加密后的Base64图像数据
  - sort_order: 整数类型，默认值0，用于图像排序
  - created_at: 文本类型，默认当前时间，记录创建时间

**约束规则**：
- 主键约束：确保每条图像记录的唯一性
- 外键约束：pwd_id引用pwd_info表的id
- 非空约束：pwd_id、file_name、file_size、mime_type、data字段必须非空
- 默认约束：sort_order默认值为0，created_at默认为当前时间
...
- [initSql.ts:107-120](file://electron/db/sqlite/components/initSql.ts#L107-L120)
- [initSql.ts:159-173](file://electron/db/sqlite/components/initSql.ts#L159-L173)
- [image.ts:6-67](file://electron/db/sqlite/mapper/image.ts#L6-L67)
- [type.ts:93-114](file://src/components/type.ts#L93-L114)
- [useDBImage.ts:18-132](file://src/hooks/useDBImage.ts#L18-L132)
...
- pwd_image表的pwd_id字段引用pwd_info表的id
- 支持级联删除，当密码信息删除时，相关图像附件也会被清理
...
#### 非空约束
- 关键字段如title、group_id、file_name、file_size、mime_type、data等设置了非空约束
- 保证图像附件数据的基本完整性
...
- **新增** pwd_image表的pwd_id字段用于按密码信息查询图像附件

#### 性能考虑
- 避免在高频更新的字段上建立过多索引
- 考虑到SQLite的轻量级特性，保持索引数量适中
- **新增** 为pwd_image表的pwd_id字段建立了专门的索引以优化查询性能
...
- 合理利用索引提高查询效率
- 避免SELECT *，只选择需要的字段
- **新增** 为图像附件查询建立了专门的索引优化

### 内存管理
- 及时关闭数据库连接
- 合理使用事务减少磁盘I/O
- 控制查询结果集大小
- **新增** 图像数据采用加密存储，避免频繁解密操作

### 扩展性考虑
- 支持未来添加新的配置项
- 灵活的分组结构设计
- 可扩展的快捷键系统
- **新增** 图像附件的可扩展设计，支持未来功能增强
...
- **症状**: 图像无法显示或加载缓慢
- **原因**: 图像数据过大或索引缺失
- **解决**: 检查图像文件大小限制，确保pwd_id索引正常工作

**章节来源**
- [baseSql.ts:12-81](file://electron/db/sqlite/components/baseSql.ts#L12-L81)
- [initSql.ts:131-174](file://electron/db/sqlite/components/initSql.ts#L131-L174)
- [image.ts:1-68](file://electron/db/sqlite/mapper/image.ts#L1-L68)
...
- **扩展性强**: 支持未来功能扩展和配置增加
- **安全性高**: 采用参数化查询防止SQL注入
- **新增** **功能完整**: 新增图像附件功能，支持加密存储和排序管理

### 业务价值
- **用户体验**: 提供流畅的密码管理体验
- **数据安全**: 采用加密存储保护敏感信息
- **系统稳定**: 完善的错误处理和恢复机制
- **新增** **多媒体支持**: 支持图像附件功能，提升用户体验

### 发展方向
- **功能扩展**: 支持更多云存储服务
- **性能优化**: 持续改进查询性能
- **安全增强**: 加强数据保护机制
- **新增** **图像处理**: 支持更多图像格式和处理功能

通过合理的数据库设计和完善的架构实现，密码管理器为用户提供了安全、可靠、易用的密码管理解决方案，新增的图像附件功能进一步增强了系统的实用性和用户体验。