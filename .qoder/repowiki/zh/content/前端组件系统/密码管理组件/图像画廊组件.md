...
**更新** 组件现已完全集成useDBImage Hook，实现了从文件选择到数据库存储的完整数据流，包括粘贴上传、预览和管理功能的完整实现。
...
- [sqlite-ipc.ts:1-295](file://electron/db/sqlite/sqlite-ipc.ts#L1-L295)
...
**更新** 组件现已完全集成useDBImage Hook，实现了从文件处理到数据库存储的完整数据流，包括粘贴事件监听、文件验证、加密存储和同步通知的完整流程。
...
**更新** 新增了粘贴上传的完整流程，包括粘贴事件监听、文件提取、格式验证和上传处理的完整序列图。
...
- [sqlite-ipc.ts:241-244](file://electron/db/sqlite/sqlite-ipc.ts#L241-L244)
...
**更新** 新增了粘贴上传事件处理的生命周期管理，包括粘贴事件监听、组件卸载时的资源清理等。
...
**更新** 新增了粘贴上传的特殊处理流程，包括粘贴事件检测、截图提取、文件名生成和统一的processFile处理。
...
**更新** 新增了粘贴上传的完整流程，包括粘贴事件监听、截图检测、文件提取和统一的上传处理。
...
UseDBImage --> Crypto : 使用加密功能
UseDBImage --> IPCRenderer : 通过IPC通信
```

**更新** 新增了完整的数据库操作接口，包括图片插入、删除、查询、计数等核心功能，以及与Electron IPC层的完整通信协议。
...
**更新** 新增了完整的加密存储流程，包括文件转Base64、数据加密、数据库存储和解密获取的完整机制。

**章节来源**
- [useDBImage.ts:18-35](file://src/hooks/useDBImage.ts#L18-L35)
- [useDBImage.ts:73-79](file://src/hooks/useDBImage.ts#L73-L79)
...
**更新** 新增了完整的数据库初始化流程，包括表结构检查、索引创建和默认数据插入的详细步骤。

**图表来源**
- [initSql.ts:108-125](file://electron/db/sqlite/components/initSql.ts#L108-L125)
- [initSql.ts:151-174](file://electron/db/sqlite/components/initSql.ts#L151-L174)

**章节来源**
- [image.ts:1-68](file://electron/db/sqlite/mapper/image.ts#L1-L68)
- [sqlite-ipc.ts:239-292](file://electron/db/sqlite/sqlite-ipc.ts#L239-L292)
...
**更新** 新增了useDBImage Hook与Electron IPC层的完整依赖关系，包括常量定义、数据库映射器和SQLite数据库的完整链路。

**图表来源**
- [ImageGallery.vue:1-20](file://src/components/indexview/ImageGallery.vue#L1-L20)
- [useDBImage.ts:1-16](file://src/hooks/useDBImage.ts#L1-L16)
- [sqlite-ipc.ts:1-70](file://electron/db/sqlite/sqlite-ipc.ts#L1-L70)
...
**更新** 新增了useDBImage Hook对加密库的依赖，以及完整的外部库依赖分析。
...
**更新** 新增了粘贴上传场景下的内存管理策略，包括粘贴事件监听的注册和清理机制。
...
**更新** 新增了useDBImage Hook的批量操作能力，包括批量删除、批量导入和全量查询的优化策略。
...
**更新** 新增了粘贴上传的特定问题排查，包括粘贴事件监听失效、截图格式不支持等问题的解决方案。
...
**更新** 新增了useDBImage Hook的同步问题排查，包括IPC通信失败、数据库操作异常等问题的诊断方法。

**章节来源**
- [ImageGallery.vue:95-118](file://src/components/indexview/ImageGallery.vue#L95-L118)
- [useDataSync.ts:68-75](file://src/hooks/useDataSync.ts#L68-L75)
