...
- 数据验证、事务处理与错误恢复机制
- **新增**：图像附件功能的完整实现方案，包括加密存储、MIME类型处理、文件大小跟踪等
...
- [image.ts:1-68](file://electron/db/sqlite/mapper/image.ts#L1-L68)
...
- 初始化脚本（initSql.ts）
  - 首次运行检测表是否存在，不存在则创建表结构与默认数据；包含版本表的创建与初始化；**新增**：图像表的创建与索引建立。
...
## 架构总览
Mapper层遵循"面向实体的DAO"模式：每个Mapper封装对应表的CRUD操作，内部仅依赖基础SQL组件，不直接持有业务逻辑，保持高内聚低耦合。前端通过Hook层发起请求，经IPC桥接至主进程的Mapper执行数据库操作。**新增**：图像映射器提供完整的图片附件管理功能，支持加密存储、元数据管理和批量操作。
...
- SQL与参数
  - 插入：INSERT INTO "pwd_image"(pwd_id, file_name, file_size, mime_type, data, sort_order) VALUES (?, ?, ?, ?, ?, ?)。
  - 删除：DELETE FROM "pwd_image" WHERE id = ?；DELETE FROM "pwd_image" WHERE pwd_id = ?；DELETE FROM "pwd_image"。
  - 查询：SELECT id, pwd_id, file_name, file_size, mime_type, sort_order, created_at FROM "pwd_image" WHERE pwd_id = ? ORDER BY sort_order, id。
- 结果映射
...
**章节来源**
- [image.ts:1-68](file://electron/db/sqlite/mapper/image.ts#L1-L68)
- [type.ts:93-114](file://src/components/type.ts#L93-L114)
...
- **新增**：图像表结构
  - 包含id、pwd_id、file_name、file_size、mime_type、data、sort_order、created_at字段。
  - 建立pwd_id索引以优化查询性能。
  - 支持默认时间戳和排序字段。
...
- 常见问题
  ...
  - **新增**：图像相关问题
    - 图片无法显示：检查MIME类型是否正确、数据是否已加密。
    - 图片上传失败：检查文件大小限制、格式支持、存储空间。
    - 图片同步异常：确认加密数据完整性、网络连接状态。
- 错误处理
...
## 结论
本Mapper层通过"基础SQL组件 + 实体Mapper"的架构，实现了清晰的职责划分与良好的扩展性。所有CRUD操作均采用参数化SQL与统一错误处理，配合初始化脚本与前端Hook层，满足密码管理器的核心数据访问需求。**新增**的图像附件功能提供了完整的图片管理解决方案，包括加密存储、元数据管理、批量操作等功能，进一步增强了系统的实用性。后续可在事务、索引与复杂联表查询等方面进一步增强。
...
- **新增**：场景五：图片附件管理
  - 步骤
    ...
    - 文件转换为Base64并加密后存储到数据库。
    - 显示图片缩略图，按需加载完整图片数据。
  - 关键路径
    - [useDBImage.ts:21-35](file://src/hooks/useDBImage.ts#L21-L35)
...
- [type.ts:9-114](file://src/components/type.ts#L9-L114)

### 图像功能技术规范

#### 图像存储策略
- **加密存储**：图片数据采用AES加密后存储，确保数据安全。
- **Base64编码**：原始二进制数据转换为Base64字符串存储。
- **元数据分离**：图片数据与元数据分开存储，支持按需加载。

#### 图像处理流程
1. **上传阶段**：前端验证文件格式、大小，转换为Base64
2. **加密阶段**：使用对称加密算法加密Base64数据
3. **存储阶段**：将加密数据写入数据库，同时记录元数据
4. **加载阶段**：按需解密并返回给前端显示
...
- [constant.ts:43-52](file://electron/constant.ts#L43-L52)