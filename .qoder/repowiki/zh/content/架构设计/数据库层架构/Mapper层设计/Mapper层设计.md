...
## 项目结构
Mapper层位于Electron主进程侧，采用“基础SQL组件 + 具体Mapper”的分层设计。基础组件提供通用的增删改查能力；具体Mapper聚焦于各业务实体（config、group、pwdInfo、oss、shortcutKey、version）的CRUD封装；初始化脚本负责首次运行时的表结构与默认数据创建。
...
- 初始化脚本（initSql.ts）
  - 首次运行检测表是否存在，不存在则创建表结构与默认数据；包含版本表的创建与初始化。
...
## 架构总览
Mapper层遵循“面向实体的DAO”模式：每个Mapper封装对应表的CRUD操作，内部仅依赖基础SQL组件，不直接持有业务逻辑，保持高内聚低耦合。前端通过Hook层发起请求，经IPC桥接至主进程的Mapper执行数据库操作。
...
### 初始化与默认数据（initSql.ts）
- 表结构
  - group、pwd_info、config、shortcut_key、oss、update_version。
- 默认数据
...
## 结论
本Mapper层通过“基础SQL组件 + 实体Mapper”的架构，实现了清晰的职责划分与良好的扩展性。所有CRUD操作均采用参数化SQL与统一错误处理，配合初始化脚本与前端Hook层，满足密码管理器的核心数据访问需求。后续可在事务、索引与复杂联表查询等方面进一步增强。
...
图表来源
- [initSql.ts:51-127](file://electron/db/sqlite/components/initSql.ts#L51-L127)
- [type.ts:9-88](file://src/components/type.ts#L9-L88)