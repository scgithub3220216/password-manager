...
export default function () {
    ...
    async function delPwdInfoByGroupId(groupId: number) {
        console.log(`useDBPwdInfo.ts delPwdInfoByGroupId groupId:${groupId}`)
        const res = await window.ipcRenderer.invoke(IPC_SQLITE_DELETE_ALL_PWD_INFO_BY_GROUP_ID,groupId);
        refreshCache()
        return res;
    }
